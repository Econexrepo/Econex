...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadAgriEffects = async () => {
      try {
        const res = await api.get(
          '/api/agriculture/charts/agri-effect-only?horizon=long_run&top_n=50'
        )
        setAgriEffects(res.data.data || [])
      } catch (err) {
        console.error('Agriculture effect chart API error', err)
  ...
  return (
    <div className="dashboard">
      <div className="charts-grid">
        ...
        {Array.isArray(agriEffects) && agriEffects.length > 0 && (
          <div className="chart-card">
            <h3 className="chart-title">Top Long-Run Agricultural Effects</h3>

            {agriUnitTabs.length > 0 && (
              <div className="agri-tabs" role="tablist" aria-label="Agriculture unit tabs">
                {agriUnitTabs.map((unit) => (
                  <button
                    key={unit}
                    type="button"
                    className={`agri-tab ${activeAgriUnit === unit ? 'active' : ''}`}
                    onClick={() => setSelectedAgriUnit(unit)}
                  >
                    {unit}
                  </button>
                ))}
            ...
            <div style={{ height: 420 }}>
              <Bar
                data={agriEffectDataset(filteredAgriEffects)}
                options={agriEffectOpts(filteredAgriEffects)}
              />
            </div>
          </div>
        )}
        ...
        {Array.isArray(agriShortRun) && agriShortRun.length > 0 && (
          <div className="chart-card">
            <h3 className="chart-title">Short-Run Agricultural Effects</h3>

            {shortRunUnitTabs.length > 0 && (
              <div
                className="agri-tabs"
                role="tablist"
                aria-label="Short-run agriculture unit tabs"
              >
                {shortRunUnitTabs.map((unit) => (
                  <button
                    key={unit}
                    type="button"
                    className={`agri-tab ${activeShortRunUnit === unit ? 'active' : ''}`}
                    onClick={() => setSelectedShortRunUnit(unit)}
                  >
                    {unit}
                  </button>
                ))}
            ...
            <div style={{ height: 420 }}>
              <Bar
                data={agriShortRunDataset(filteredAgriShortRun)}
                options={agriShortRunOpts(filteredAgriShortRun)}
              />
            </div>
          </div>
        )}
